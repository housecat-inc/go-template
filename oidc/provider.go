...
func NewProvider(issuer string, db *sql.DB, keyPath string, sessionSecret string) (op.OpenIDProvider, *Storage, error) {
	...
	if sessionSecret == "" {
		...
		if _, err := rand.Read(randomKey[:]); err != nil {
			return nil, nil, errors.Wrap(err, "generate random crypto key")
		}
	...
	if err != nil {
		return nil, nil, errors.Wrap(err, "create oidc provider")
	}
